...
public class User {
    ...
    @Id
    @GeneratedValue(strategy = GenerationType.AUTO)  // can give Identity also
    private Long id;
    ...
    @Embedded  // because this is not an entity
    @ElementCollection // this will create another talble for it
    @CollectionTable(name = "payment_information",joinColumns = @JoinColumn(name = "user_id"))  // we can change the names of rows and columns of table with this annotation
    private List<PaymentInformation> paymentInformations = new ArrayList<>();

    @OneToMany(mappedBy = "user",cascade = CascadeType.ALL)
    @JsonIgnore // because it has user entity --> can cause nested problems
